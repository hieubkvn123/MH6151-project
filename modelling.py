...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, required=False, 
            choices=['decision_tree', 'random_forest', 'gradient_boost_tree', 'adaboost', 'xgboost', 
                     'voting_naive_bayes', 'stacking_naive_bayes', 'gaussian_naive_bayes'],
    ...
    if(args['model_name'] not in ['voting_naive_bayes', 'stacking_naive_bayes', 'gaussian_naive_bayes']):
        train_df = preproc_df_for_tree_algos(train_df) 
        test_df  = preproc_df_for_tree_algos(test_df)
    else:
        train_df = preproc_df_for_bayes_algos(train_df) 
        test_df  = preproc_df_for_bayes_algos(test_df)
    ...
    if(args['oversampling']):
        ...
        CHECKPOINT_FOLDER = os.path.join(CHECKPOINT_FOLDER, 'tuned_with_oversampling')
        pathlib.Path(CHECKPOINT_FOLDER).mkdir(parents=True, exist_ok=True)
        ...
        # Create a separate folder
        CHECKPOINT_FOLDER = os.path.join(CHECKPOINT_FOLDER, 'tuned')
        pathlib.Path(CHECKPOINT_FOLDER).mkdir(parents=True, exist_ok=True)
    ...
    results, best_param = hyperparams_tuning(X_train.copy(), Y_train.copy(), model_class, 
                                             hyperparams, metrics, target_metric='mcc', model_name=args['model_name'])
